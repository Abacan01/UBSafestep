...
class _NotificationsScreenState extends State<NotificationsScreen> {
    ...
    setState(() {
      ...
      if (filter == 'All') {
        ...
        _filteredNotifications = _notifications
            ...
            .toList();
      } else if (filter == 'Predefined Zone') {
        _filteredNotifications = _notifications
            .where((notification) {
              final type = _getNotificationType(notification);
              return type == 'Predefined Zone';
            })
            .toList();
        ...
        notification['EmergencySOS'] == false &&
            ...
            _getNotificationType(notification) != 'Predefined Zone')
            .toList();
  ...
  String _getNotificationType(Map<String, dynamic> notification) {
    ...
    // Check if it's a predefined zone notification
    final predefinedZones = _firestoreService.getPredefinedSafezones();
    final isPredefinedZone = predefinedZones.any((zone) {
      final zoneName = zone['Zonename']?.toString().toLowerCase() ?? '';
      return message.contains(zoneName.toLowerCase()) || 
             message.contains('university of batangas');
    });
    
    if (isPredefinedZone && (message.contains('entered') || message.contains('left'))) {
      return 'Predefined Zone';
  ...
  Color _getNotificationColor(String type, Map<String, dynamic> notification) {
    ...
    if (type == 'Predefined Zone') {
      final message = notification['Message']?.toString().toLowerCase() ?? '';
      if (message.contains('entered')) {
        return Colors.green; // Entering predefined zone is green
      } else if (message.contains('left')) {
        return Colors.orange; // Leaving predefined zone is orange
      }
      return Colors.purple; // Default for predefined zones
    }
  ...
  IconData _getNotificationIcon(String type, Map<String, dynamic> notification) {
    ...
    if (type == 'Predefined Zone') {
      final message = notification['Message']?.toString().toLowerCase() ?? '';
      if (message.contains('entered')) {
        return Icons.school; // Entering predefined zone (school)
      } else if (message.contains('left')) {
        return Icons.school_outlined; // Leaving predefined zone
      }
      return Icons.school;
    }
          ...
          Container(
            ...
            child: ListView(
              ...
              children: [
                ...
                const SizedBox(width: 8),
                _buildFilterChip('Predefined Zone', _currentFilter == 'Predefined Zone', Colors.purple),
                const SizedBox(width: 8),
