  ...
  Future<void> _loadStudentData({bool isRefresh = false}) async {
    ...
    try {
      ...
      if (mounted && studentData != null) {
        // Parent‑defined safezones from Firestore
        final parentSafezones = await _firestoreService.getSafezonesByParent(widget.userId);
        // Built‑in UB predefined safezones (static list)
        final predefinedSafezones = _firestoreService.getPredefinedSafezones();

        // Consider BOTH parent safezones and predefined UB safezones
        final bool anySafezonesExist =
            parentSafezones.isNotEmpty || predefinedSafezones.isNotEmpty;
        studentData['safezonesExist'] = anySafezonesExist;
