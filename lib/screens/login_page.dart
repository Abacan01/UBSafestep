...
class _LoginPageState extends State<LoginPage> {
  ...
  void _handleSignUp() async {
    ...
    // Additional year validation check before proceeding
    final yearValidation = _validateEmailYear(email);
    if (yearValidation != null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(yearValidation),
          backgroundColor: Colors.red,
  ...
  String? _validateUbStudentEmail(String? value) {
    ...
    // Check if email year is 1999 or below
    final yearValidation = _validateEmailYear(email);
    if (yearValidation != null) {
  ...
  // Validate email year - must be 2000 or above
  String? _validateEmailYear(String email) {
    try {
      // Extract student ID (numbers before @)
      final studentId = email.split('@').first;
      
      // Check if student ID has at least 4 digits (year format)
      if (studentId.length < 4) {
        return 'Invalid student ID format';
      }
      
      // Extract first 4 digits as year
      final yearString = studentId.substring(0, 4);
      ...
      // Block account creation if year is 1999 or below
      if (year <= 1999) {
        return 'Account creation is not allowed for emails created in 1999 or earlier. Please contact support.';
      }
      
      return null;
    } catch (e) {
      print('Error validating email year: $e');
      return 'Error validating email year';
    }
  }
