...
class _SafeZonesScreenState extends State<SafeZonesScreen> {
  ...
  Future<void> _saveSafeZone() async {
    ...
    try {
      ...
      // Re-check student location against the new safezone
      // This will trigger a notification if the student is now inside the safezone
      try {
        ...
        if (studentId != null && studentId.isNotEmpty) {
          ...
          if (studentData != null) {
            final lastLat = studentData['lastLatitude'];
            final lastLng = studentData['lastLongitude'];
            
            if (lastLat != null && lastLng != null) {
              print('🔄 [SAFEZONES] Re-checking student location after safezone creation...');
              await _locationMonitor.checkAndNotifySafezoneStatus(
                studentId: studentId,
                parentGuardianId: widget.userId,
                studentLat: (lastLat as num).toDouble(),
                studentLng: (lastLng as num).toDouble(),
              );
              print('✅ [SAFEZONES] Location re-check completed');
            }
          }
        }
      } catch (e) {
        print('⚠️ [SAFEZONES] Error re-checking location: $e');
